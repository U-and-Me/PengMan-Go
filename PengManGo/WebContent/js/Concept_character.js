...
let Img_list = ['cinderella', 'elsa', 'mermaid','rapunzel', 'sleepingBeauty', 'snowwhite'];
...
function checkAlpha(clicked_id){
    ...
    if(Ans_Right == word_len && imgAdd <= Lev_Img[word - 1]){
        ...
        if((wordCount + 1) == word){
            ...
            // 모든 스테이지를 꺴을 경우
            if(cur_stage == 31){
            	localStorage.setItem("character_stage", cur_stage);
            	var link = './html/story/story_character.html';
    			location.href = link;
    			location.replace(link);
    			window.open(link);
            }else{           
			// 스테이지 페이지로 이동
			var link = './Level_character.jsp?stage=' + encodeURI(cur_stage);
    		location.href = link;
    		location.replace(link);
    		window.open(link);
    		}
...
function AddImg(){
    ...
    img.src = './img/character_'+Img_list[Img_Arr[ind]]+'.png';
    img.id = 'animal_img';
