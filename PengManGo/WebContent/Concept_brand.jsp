...
<head>
    ...
    <title>PengMan🐧</title>
    <link rel="stylesheet" href="css/Concept_game.css?testNm=8">
    <script src="js/Concept_brand.js?testNm=8" defer></script>
</head>
	...
	// 1~10 : 1개 / 11 ~ 20 : 2개 / 21 ~ 30 : 3개
		...
		String sql = "select c_word from word where diff='1' and concept='brand' order by rand() limit 1";
		...
		sql = "select c_word from word where diff='2' and concept='brand' order by rand() limit 2";
		pstmt = conn.prepareStatement(sql);
		rs = pstmt.executeQuery();
		while(rs.next()){
		...
		sql = "select c_word from word where diff='3' and concept='brand' order by rand() limit 3";
		pstmt = conn.prepareStatement(sql);
		rs = pstmt.executeQuery();
		while(rs.next()){
